...
class Logger {
    ...
    command (method, path) {
        if (method && path) {
            ...
            const sessionId = path.match(/[a-z0-9]{40}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{12}/)
            if (sessionId) {
    ...
    data (data) {
        ...
        if (data && (this.logLevel === 'data' || this.logLevel === 'verbose')) {
            const sessionId = this._sessionIdStorage ? this._sessionIdStorage : ''
            this.log(`${COLORS.brown}DATA\t\t${COLORS.reset} ${sessionId} ${data}`)
        }
    }
    ...
    result (result) {
        ...
        const sessionId = this._sessionIdStorage ? this._sessionIdStorage : ''
        this.log(`${COLORS.teal}RESULT\t\t${COLORS.reset} ${sessionId} ${result}`)
    }
    ...
    error (msg) {
        ...
        if (msg) {
            const sessionId = this._sessionIdStorage ? this._sessionIdStorage : ''
            this.log(COLORS.red + 'ERROR\t' + COLORS.reset + msg + sessionId, null)
        }
    }
